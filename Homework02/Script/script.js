...
registerBtn.addEventListener('click', function(ev) {
    ...
    for (const student of listOfStudents) {
        ...
        td6.innerText = student.getAge();
        td7.innerText = student.getGradesAverage();
...
function isRegistered(newStudent) {
    for(const student of listOfStudents) {
        if(newStudent.firstName === student.firstName && newStudent.lastName === student.lastName && newStudent.birthYear === student.birthYear) {
            return true;
        }
